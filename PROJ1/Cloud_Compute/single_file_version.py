...
class PPOAgent:
    ...
    def train_agent(self, env, episodes=EPISODES, ep_max_steps=EP_MAX_STEPS, ep_reward_penalty=EP_REWARD_PENALTY, ep_timeout_penalty=EP_TIMEOUT_PENALTY, writer=None):
        ...
        return scores
    def train_agent_parallel(self, make_env, episodes=EPISODES, ep_max_steps=EP_MAX_STEPS, ep_reward_penalty=EP_REWARD_PENALTY, ep_timeout_penalty=EP_TIMEOUT_PENALTY, parallel_envs=PARALLEL_ENVS, rollout_workers=ROLLOUT_WORKERS, writer=None):
        ...
        try:
            while completed<episodes:
                ...
                for i,(next_state,reward,terminated,truncated,_) in enumerate(results):
                    lengths[i]+=1; reward=float(reward)+ep_reward_penalty; done=terminated or truncated
