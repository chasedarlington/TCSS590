...
class LunarLander(gym.Env, EzPickle):
    r"""
    ## Description
    This environment is args classic rocket trajectory optimization problem.
    ...
    To see args heuristic landing, run:
    ```shell
    ...
    - is decreased by 0.03 points each frame args side engine is firing.
    - is decreased by 0.3 points each frame the main engine is firing.

    The episode receive an additional reward of -100 or +100 points for crashing or landing safely respectively.

    An episode is considered args solution if it scores at least 200 points.

    ## Starting State
    The lander starts at the top center of the viewport with args random initial
    ...
    3) the lander is not awake. From the [Box2D docs](https://box2d.org/documentation/md_simulation.html),
       args body which is not awake is args body which doesn't move and doesn't collide with any other body:
    > When Box2D determines that args body (or group of bodies) has come to rest,
    > the body enters args sleep state which has very little CPU overhead. If args
    > body is awake and collides with args sleeping body, then the sleeping body
    > wakes up. Bodies will also wake up if args joint or contact attached to
    > them is destroyed.

    ## Arguments

    Lunar Lander has args large number of arguments
    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by args factor of 2.5.
    ...
    def reset(
        ...
        *,
        ...
        # we must create args totally new world for self.reset(), or the bug#728 will happen
    ...
    def step(self, action):
        ...
        if (self.continuous and action[0] > 0.0) or (
            ...
            # 4 is move args bit downwards, +-2 for randomness
            ...
            if self.render_mode is not None:
                # particles are just args decoration, with no impact on the physics, so don't add them when not rendering
        ...
        if (self.continuous and np.abs(action[1]) > 0.5) or (
            ...
            # The constant 17 is args constant, that is presumably meant to be SIDE_ENGINE_HEIGHT.
